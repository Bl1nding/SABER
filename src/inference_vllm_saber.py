...
class SABEREarlyExitEngine:
    def __init__(self, engine: AsyncLLMEngine, tokenizer, args):
        ...
        self.rss_threshold = args.rss_threshold
    ...
    @staticmethod
    def compute_sc(neutral_answers: List[str], adversarial_answers: List[str]) -> float:
        """
        Computes the multiset Jaccard similarity between answers sampled
        from the neutral and adversarial branches.
        """
        from collections import Counter
        # Normalize answers before comparing their distributions.
        BOX='\\boxed{'
        neutral_clean = [clean_answer(BOX+a+'}') for a in neutral_answers]
        adversarial_clean = [clean_answer(BOX+a+'}') for a in adversarial_answers]
        
        c1 = Counter(neutral_clean)
        c2 = Counter(adversarial_clean)

        keys = set(c1) | set(c2)
        if not keys: return 0.0

        overlap = sum(min(c1[k], c2[k]) for k in keys)
    ...
    async def _probe_branch(self, prefix_ids: List[int], suffix_ids: List[int], n: int = 4) -> List[Tuple[str, float, int]]:
        ...
        Each result contains the generated text, geometric mean token
        ...
        # Collect text, confidence, and token count for each sample.
        results = []
        for output in final_res.outputs:
            ...
            for lp_dict in logprobs:
                if lp_dict:
                    # Use the highest returned token probability.
                    max_lp = max(obj.logprob for obj in lp_dict.values())
                    sum_logprobs += max_lp
                    valid_tokens += 1
                    
            geom_mean = math.exp(sum_logprobs / valid_tokens) if valid_tokens > 0 else 0.0
            results.append((gen_text, geom_mean, actual_len))
        ...
        async with sem:
            ...
            while len(generated_ids) < max_gen_budget:
                ...
                if not has_closed_think and current_step_tokens_count >= self.min_step_tokens:
                    ...
                    # Measure semantic consistency across the two branches.
                    neutral_answers = [r[0] for r in neutral_results]
                    adversarial_answers = [r[0] for r in adversarial_results]
                    sc = self.compute_sc(neutral_answers, adversarial_answers)
                    
                    # Measure confidence stability with exponential decay.
                    avg_p_neutral = np.mean([r[1] for r in neutral_results]) if neutral_results else 0.0
                    avg_p_adversarial = np.mean([r[1] for r in adversarial_results]) if adversarial_results else 0.0
                    
                    cs = np.exp(-self.gamma * abs(avg_p_neutral - avg_p_adversarial))
                    
                    # Combine both signals into the reasoning stability score.
                    rss = self.alpha * sc + (1 - self.alpha) * cs

                    
                    # Stop reasoning once the current state is sufficiently stable.
                    if rss >= self.rss_threshold:
                        # tqdm.write(
                        #     f"[SABER Early Exit] Idx: {idx} | RSS: {rss:.4f} | "
                        #     f"SC: {sc:.2f} | CS: {cs:.2f} | "
                        #     f"ConfidenceDiff: {abs(avg_p_neutral-avg_p_adversarial):.4f}"
                        #     )
...
async def run_evaluation(args):
    ...
    executor = SABEREarlyExitEngine(engine, tokenizer, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rss_threshold", "--trs_threshold",
        dest="rss_threshold", type=float, default=0.9,
        help="SABER Reasoning Stability Score (RSS) threshold tau",
    )
    parser.add_argument("--alpha", type=float, default=0.5, help="Weight assigned to SC in RSS")
